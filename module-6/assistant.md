...
让我们更新此助手，为其添加我的 `user_id` 以便使用，并[为其创建新版本](https://docs.langchain.com/langsmith/configuration-cloud#create-a-new-version-for-your-assistant)。
...
Now, let's create a work assistant.

现在，让我们创建一个工作助手。

I'll use this for my work tasks.
...
这使我们能通过 SDK 轻松搜索<!--[~search~](https://langchain-ai.github.io/langgraph/cloud/how-tos/configuration_cloud/)--> [搜索](https://reference.langchain.com/python/langsmith/deployment/sdk/#langgraph_sdk.client.AssistantsClient.search) 助手。
...
We can manage them easily with the SDK.

我们可以通过 SDK 轻松管理它们。

For example, we can delete assistants that we're no longer using.
> The syntax in the video is slightly off.

例如，我们可以删除不再使用的助手。
> 视频中的语法略有偏差。

The updated code below creates a spare assistant and then deletes it.

下方更新后的代码先创建一个备用助手，然后将其删除。
...
让我们为我将要使用的 `personal` 和 `work` 助手设置其 ID。
